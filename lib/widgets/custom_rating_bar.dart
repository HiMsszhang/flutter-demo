...
class RatingBar extends StatefulWidget {
  ...
  RatingBar({this.starCount = 5, this.rating = 0.0, this.color, this.icon, this.size, this.onRatingCallback, this.clickedCallbackAsIndicator, this.isIndicator = false, this.spacing = 0.0, this.allowHalfRating = true, this.showAllStar = true}) {
              ...
              return ValueListenableBuilder(
                  builder: (BuildContext context, double value, Widget child) {
                    return Wrap(
                      ...
                      children: List.generate(widget.starCount, (index) => buildStar(context, index, value, isIndicator)),
                    );
                  },
  ...
  Widget buildStar(BuildContext context, int index, double rating, bool isIndicator) {
    ...
    if (index >= rating) {
      ...
      container = Container(
        ...
        height: widget.size,
        child: Opacity(
          opacity: widget.showAllStar ? 1 : 0,
          child: widget.icon,
        ),
      );
    } else if ((index >= rating - (widget.allowHalfRating ? 0.5 : 1.0) && index < rating) && widget.allowHalfRating) {
