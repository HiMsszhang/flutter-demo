...
DateDart wrapper(DateTime date) {
  return DateDart.fromDateTime(date);
}
...
class DateDart {
  ...
  DateDart.fromDateTime(DateTime date) {
    this._date = date;
  }

  DateDart.fromString(String iso8601) {
    this._date = DateTime.parse(iso8601);
  }

  DateDart.fromInt(int year, [int month = 1, int day = 1, int hour = 0, int minute = 0, int second = 0, int millisecond = 0, int microsecond = 0]) {
    this._date = DateTime(year, month, day, hour, minute, second, millisecond, microsecond);
  }

  static bool isDateDart(daydart) {
    return daydart is DateDart;
  }
  ...
  ///检测当前 DateDart 对象是一个有效的 DateTime
  bool isValid() {
  ...
  /// 复制 DateDart 对象
  DateDart clone([DateTime dateTime]) {
    if (dateTime != null) {
  ...
  /// 检查另外一个 DateDart 对象是否与当前 DateDart 对象的时间相等
  bool isSame(DateDart that) {
    return _date.isAtSameMomentAs(that.toDateTime());
  }

  /// 检查另外一个时间数值 是否与当前 DateDart 相应枚举相等
  ...
  /// 检查另外一个 DateDart 对象是否在当前 DateDart 对象时间之后
  bool isAfter(DateDart that) {
    return _date.isAfter(that.toDateTime());
  }

  /// 检查另外一个时间数值 是否在当前 DateDart 相应枚举之后
  ...
  /// 检查另外一个 DateDart 对象是否在当前 DateDart 对象时间之前
  bool isBefore(DateDart that) {
    return _date.isBefore(that.toDateTime());
  }

  /// 检查另外一个时间数值 是否在当前 DateDart 相应枚举之前
  ...
  /// 操作时间-增加
  DateDart add(int num, Units unit) {
    int y = year();
    int m = month();
    int d = date();
    int h = hour();
  ...
  /// 操作时间-减少
  DateDart subtract(int num, Units unit) {
    int y = year();
    int m = month();
    int d = date();
    int h = hour();
  ...
  /// 格式化时间戳
  static String formatTimestamp(int timestamp, String mode) {
    var time = DateTime.fromMillisecondsSinceEpoch(timestamp);
    var formatter = new DateFormat(mode ?? 'yyyy-MM-dd HH:mm');
